...
//typedef stdext::hash_map<EdgeKey, int> EdgeMapInt;
//typedef stdext::hash_map<EdgeKey, Vector2i> adjacentTrianglesIdxEdgeMapVec2i;
	...
	for(int i=0; i<numE; i++){
		if(edgeSharedTriangle[i].size()>1){
			...
			for(int i=0; i<3; i++) {
				...
				for(int j=0; j<3; j++) {
					if(triangles[idx1].vindices[i] == triangles[idx2].vindices[j]) bSharedVertex = true;
				}
				if(!bSharedVertex) {
					...
					v[5] = idx2;
					for(int p =0; p<3; ++p) {
						if(triangles[idx2].vindices[p] != v[2] && triangles[idx2].vindices[p] != v[3]) {
			...
			for(int j=0; j<6; j++) adjacentTrianglesIdx.push_back(v[j]);
		}
	}
}